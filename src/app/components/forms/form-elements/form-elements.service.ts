...
interface GithubUser {
    // Define the properties you expect from the API response
    id: number;
    login: string;
    avatar_url: string;
    // ... other properties
...
export class FormElementsService {
  ...
  getGithubAccounts(term: string | null = null): Observable<GithubUser[]> {
    if (term) {
      return this.http.get<{ items: GithubUser[] }>(`https://api.github.com/search/users?q=${term}`)
        .pipe(
          map(rsp => rsp.items)
        );
    } else {
  ...
  // Month
  getMonth(term: string | null = null): Observable<Month[]> {
  ...
  // year
  getYear(term: string | null = null): Observable<Year[]> {
  ...
  // Days 
  getDays(term: string | null = null): Observable<Days[]> {
